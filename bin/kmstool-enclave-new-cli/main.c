...
// static int encrypt(struct app_ctx *app_ctx, struct aws_byte_buf *plainttext) {
//     ssize_t rc = 0;
...
//     rc = aws_kms_encrypt_blocking(
//         client, app_ctx->key_id, , &ciphertext, &ciphertext_decrypted);
...
// }
static int generate(struct app_ctx *app_ctx, struct aws_byte_buf *plaintext_decrypted_b64) {
    ssize_t rc = 0;
    ...
    // int aws_kms_generate_random_blocking( uint32_t number_of_bytes, struct aws_byte_buf *plaintext)

    uint32_t number_of_bytes = 32;
    ...
    rc = aws_kms_generate_random_blocking(
        client, number_of_bytes , &plaintext);

    aws_byte_buf_clean_up(&plaintext);

    fail_on(rc != AWS_OP_SUCCESS, "Could not generate random value from kms");

    //todo 各种链的
    // //然后用一个key进行加密
    // struct aws_byte_buf ciphertext_decrypted;
    // rc = aws_kms_encrypt_blocking(
    //     client, app_ctx->key_id, , &plaintext, &ciphertext_decrypted);

    // fail_on(rc != AWS_OP_SUCCESS, "Could not encrypt random value from kms");

    /* Encode ciphertext into base64 for printing out the result. */
    size_t plaintext_decrypted_b64_len;
    struct aws_byte_cursor plaintext_decrypted_cursor = aws_byte_cursor_from_buf(&plaintext);
    aws_base64_compute_encoded_len(plaintext.len, &plaintext_decrypted_b64_len);
    rc = aws_byte_buf_init(plaintext_decrypted_b64, app_ctx->allocator, plaintext_decrypted_b64_len + 1);
    fail_on(rc != AWS_OP_SUCCESS, "Memory allocation error");
    rc = aws_base64_encode(&plaintext_decrypted_cursor, plaintext_decrypted_b64);
    fail_on(rc != AWS_OP_SUCCESS, "Base64 encoding error");
    aws_byte_buf_append_null_terminator(plaintext_decrypted_b64);
...
int main(int argc, char **argv) {
    ...
    struct aws_byte_buf plaintext_decrypted_b64;
    int rc;
    ...
    fprintf(stdout, "%s", (const char *)plaintext_decrypted_b64.buffer);

    aws_byte_buf_clean_up(&plaintext_decrypted_b64);
